...
const hospitalReducer = (state: Hospitals_State, action:any) => {
	switch (action.type) {
		...
		case UPDATE_HOSPITAL:
			return {
				...
				hospitals: state.hospitals.map(
					(hospital: Hospital) => hospital._id === action.payload.data._id ?
						action.payload.data :
						hospital
				),
				loading: false,
		...
		case DELETE_HOSPITAL:
			return {
				...
				hospitals: state.hospitals.filter(
					(hospital: Hospital) => hospital._id !== action.payload
				),
				loading: false,
			};
		...
		case FILTER_HOSPITALS:
			return {
				...state,
				filtered: state.hospitals.filter(
					(hospital: Hospital) => {
						const regex = new RegExp(`${action.payload}`, 'gi');
						return hospital.name.match(regex) || hospital.adress.match(regex);
					}
				),
				loading: false,
			};
